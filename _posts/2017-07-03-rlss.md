...
3. Of course, real-world environments often have an element of uncertainty; we sometimes intend to take one action, but we end up in an unintended state. So an MDP also defines a model of the dynamics of the environment. The dynamics model is a set of transition probabilities that are conditioned on the current state (as in a Markov Chain) *and* on the action: $T(s,a,s') = Pr(s_{t+1}|s_t,a_t)$. Here, $T$ is the transition probability, $s$ (or $s_t$) is the current state, $a$ is the action, and $s'$ (or $s_{t+1}$) is the future state.
